...
typedef struct STDesc
{
  ...
  Eigen::Vector3d side_length_; // 缩放后的边长a<=b<=c

  // projection angle between vertices
  Eigen::Vector3d angle_; // 三个角度

  Eigen::Vector3d center_; // 质心
  unsigned int frame_id_;  // 当前帧id

  // three vertexs
  Eigen::Vector3d vertex_A_; //  最大边相对点
  Eigen::Vector3d vertex_B_; // 次大边相对点
  Eigen::Vector3d vertex_C_; // 最短边相对点

  // some other inform attached to each vertex,e.g., intensity
  Eigen::Vector3d vertex_attached_; // 三个点的强度
} STDesc;
...
// VOXEL_LOC类代表一个3D体素的位置。
class VOXEL_LOC
{
public:
  int64_t x, y, z; // x, y, z代表体素在三维空间中的位置
  // 默认构造函数，初始化体素位置为(0, 0, 0)
  VOXEL_LOC(int64_t vx = 0, int64_t vy = 0, int64_t vz = 0)
      : x(vx), y(vy), z(vz) {}
  // 重载等号运算符，以便比较两个VOXEL_LOC是否具有相同的位置
...
// M_POINT结构代表一个点，包括其三维坐标、强度值以及一个计数器
struct M_POINT
{
  float xyz[3];    // 三维坐标
  float intensity; // 点的强度值
  int count = 0;   // 用于体素内的点数计数
};
...
// STDesc_LOC类是用来作为数据库的哈希键，它表示了描述符的位置，但只使用了三个属性来确定键的唯一性（即x, y, z）。
// 这种方式的目的是将描述符分到一个特定的桶中，以加快查找和匹配的速度。
class STDesc_LOC
{
public:
  int64_t x, y, z, a, b, c;
...
// PlaneSolver结构，它定义了一个代价函数，用于计算点到平面的距离。
struct PlaneSolver
{
  // 构造函数，初始化当前点、当前法线、目标点和目标法线
  ...
  // 代价函数操作符重载，ceres库会调用它来计算残差
  template <typename T>
  bool operator()(const T *q, const T *t, T *residual) const
  {
    // 将输入的四元数和平移向量转换为Eigen类型
